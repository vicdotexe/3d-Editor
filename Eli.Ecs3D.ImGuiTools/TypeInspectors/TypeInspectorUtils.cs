...
namespace Eli.Ecs3D.ImGuiTools.TypeInspectors
{
	public static class TypeInspectorUtils
	{
		...
		static readonly Type transformType = typeof(Transform3);
		...
		public static AbstractTypeInspector GetInspectorForType(Type valueType, object target, MemberInfo memberInfo)
		{
			...
			if (target is Entity)
				return new EntityFieldInspector();
			if (target is BlendState)
				return new BlendStateInspector();
            ...
            if (valueType == typeof(Model))
                return new ModelInspector();
            //if (valueType == typeof(HeightMap))
               // return new TerrainInspector();
