...
namespace Eli.Ecs3D
{
    ...
    public class Entity : IComparable<Entity>, ITransformable3D, IGizmoObject
    {
        ...
        public void Destroy()
        {
            ...
            {
                var child = Transform.GetChild(i).Owner;
                child.Destroy();
            }
        }
        ...
        public void DetachFromWorld()
        {
            ...
            for (var i = 0; i < Transform.ChildCount; i++)
                Transform.GetChild(i).Owner.DetachFromWorld();
        }
        ...
        public void AttachToWorld(World newScene)
        {
            ...
            for (var i = 0; i < ChildCount; i++)
                Transform.GetChild(i).Owner.AttachToWorld(newScene);
        }
        #endregion
        ...
        /// </summary>
        public virtual Entity Clone(Vector3 position = default(Vector3))
        {
        ...
        protected void CopyFrom(Entity entity)
        {
            ...
            {
                var child = entity.Transform.GetChild(i).Owner;
        ...
        public Entity SetTag(int tag)
        {
            if (_tag != tag)
            {
                ...
                if (World != null)
                    World.Entities.RemoveFromTagList(this);
                _tag = tag;
                if (World != null)
                    World.Entities.AddToTagList(this);
            }
        ...
        public Entity SetUpdateOrder(int updateOrder)
        {
            if (_updateOrder != updateOrder)
            {
                ...
                if (World != null)
                {
                    World.Entities.MarkEntityListUnsorted();
                    World.Entities.MarkTagUnsorted(Tag);
                }
            }
        ...
        {
            //AddComponent(new TransformWidget());
        }

        public void OnDeselected()
        {
            //RemoveComponent<TransformWidget>();
        }


        public BoundingBox EditorBoundingBox
        {
            get
            {
                
                //return BoundingBox.CreateFromSphere(EditorBoundingSphere);
                if (HasComponent<RenderableComponent>())
                {
                    var comp = GetComponent<RenderableComponent>();
                    if (comp.Enabled)
                        return comp.BoundingBox;
                }

                if (!HasComponent<Camera3D>())
                {
                    float size = 5f;
                    return new BoundingBox(WorldMatrix.Translation - new Vector3(size),
                        WorldMatrix.Translation + new Vector3(size));
                }
        ...
        public float? Select(Ray selectionRay)
        {
            ...
            if (model != null)
            {
                foreach (var vertices in model.ModelData.meshPartVertices)
                {
                    var ray = CollisionHelper.RayIntersectsModel(vertices, model.BoundingBox, WorldMatrix, selectionRay);
                    if (ray != null)
                        return ray.Value;
                }
            ...
            return selectionRay.Intersects(EditorBoundingBox);
        }

        public virtual void EditorRender(ICamera camera)
        {
            Graphics.DrawSphere(Matrix.CreateScale(10f) * WorldMatrix, camera.ViewMatrix, camera.ProjectionMatrix, Color.Pink);
        }
