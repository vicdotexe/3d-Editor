...
namespace Eli.Physics
{
    ...
    public interface ITransformable
    {
        ...
        Transformable Parent { get; set; }
        int ChildCount { get; }
        Transformable GetChild(int index);
    }
    ...
    public abstract class Transformable : ITransformable
    {
        ...
        internal TransformFlags _flags = TransformFlags.All; // dirty flags, set all dirty flags when created

        private Transformable _parent;
        private List<Transformable> _children = new List<Transformable>();

        public int ChildCount => _children.Count();

        public Transformable GetChild(int index)
        {
        ...
        public Transformable Parent
        {
            ...
            set
            {
                ...
                oldParentTransform?._children.Remove(this);
                _parent?._children.Add(this);
            }
        }

        private void OnParentChanged(Transformable oldParent, Transformable newParent)
        {
        ...
        internal void InvokeTransformBecameDirty()
        {
            TransformBecameDirty?.Invoke();
        }

        internal void InvokeTransformUpdated()
        {
    ...
    public abstract class BaseTransform<TMatrix> : Transformable
    ...
    ///     </para>
    /// </remarks>
    public class Transform2 : BaseTransform<Matrix2D>
    {
        private Vector2 _position;
        private float _rotation;
        private Vector2 _scale = Vector2.One;

        public Transform2(object body)
            : this(body,Vector2.Zero, 0, Vector2.One)
        {
        }

        public Transform2(object body, float x, float y, float rotation = 0, float scaleX = 1, float scaleY = 1)
            : this(body,new Vector2(x, y), rotation, new Vector2(scaleX, scaleY))
        {
        }

        public Transform2(object body, Vector2? position = null, float rotation = 0, Vector2? scale = null)
        {
        ...
        protected internal override void RecalculateWorldMatrix(ref Matrix2D localMatrix, out Matrix2D matrix)
        {
            if (Parent != null)
            {
                ((Transform2)Parent).GetWorldMatrix(out matrix);
        ...
        protected internal override void RecalculateLocalMatrix(out Matrix2D matrix)
        {
            matrix = Matrix2D.CreateScale(_scale) *
                     Matrix2D.CreateRotation(_rotation) *
    ...
    public class Transform3 : BaseTransform<Matrix>
    {
        ...
        public Transform3(object body,Vector3? position = null, Quaternion? rotation = null, Vector3? scale = null)
        {
            Body = body;
        ...
        protected internal override void RecalculateWorldMatrix(ref Matrix localMatrix, out Matrix matrix)
        {
            if (Parent != null)
            {
                ((Transform3)Parent).GetWorldMatrix(out matrix);
