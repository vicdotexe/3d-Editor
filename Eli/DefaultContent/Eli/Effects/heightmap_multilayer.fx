...
void Texture(in PS_INPUT Input, out PS_OUTPUT Output)
{
	...
	float3 LightsColor = ambientColor + float3(0,0,0);

	int z = 0;
	// process directional lights
	for (z = 0; z < DirectionalLightsCount; ++z)
	{
		// calculate angle factor
		float cosTheta = saturate(dot(LightPosition[z], Input.Normal));

		// add light to pixel
		LightsColor.rgb += (LightColor[z]) * (cosTheta * LightIntensity[z]);
	}

	// now process all point lights
	for (z = DirectionalLightsCount; z < ActiveLightsCount; ++z)
	{
		...
		float disFactor = 1.0f - (distance(Input.Position3D, LightPosition[z]) / LightRange[z]);
		...
		if (disFactor > 0)
		{
			...
			float3 lightD = normalize(Input.Position3D - LightPosition[z]);
			float cosTheta = saturate(dot(-lightD, Input.Normal));

			// add light to pixel
			LightsColor.rgb += (LightColor[z]) * (cosTheta * LightIntensity[z] * disFactor);
		}
	}

	Output.Color.rgb = max(ambientColor, ((color * LightsColor * lightPower)) / 4.0);
